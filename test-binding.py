...
show_topology = os.path.join(here, "scripts", "show_topology.sh")
report_and_run = os.path.join(here, "scripts", "report_and_run.sh")
bash = shutil.which("bash")

for script in [show_topology, report_and_run]:
...
def discover_topology(faux_xml=None):
    ...
    for line in result.stdout.strip().split('\n'):
        if not line.startswith("NUMA_NODE"): continue
...
def stream(cmd):
    ...
    stdout_thread = threading.Thread(target=reader_thread, args=(process.stdout, captured_stdout, ""))
    stderr_thread = threading.Thread(target=reader_thread, args=(process.stderr, captured_stderr, ""))
    stdout_thread.start()
    stderr_thread.start()
    stdout_thread.join()
    stderr_thread.join()
...
def predict_layout(topology, num_tasks, cores_per_task=None, match_policy="high"):
    """
    ...
    TODO: Dan - probably you know what should go in here.
    """
    ...
    if num_tasks * cores_per_task > len(available_cores):
        print(f"Warning: Requesting more total cores ({num_tasks * cores_per_task}) than available ({len(available_cores)}).")
    ...
    for i in range(num_tasks): # For each task (rank)
        ...
        if core_idx < len(available_cores):
            ...
            core_id_str = subprocess.run(["hwloc-calc", "--pulist", cpuset], check=True, capture_output=True, text=True).stdout.strip()
...
def run_and_capture(command, nodes, tasks, exclusive=False, cores_per_task=None):
    """
    ...
    cmd = [
        "flux", "run",
        # --cores-per-task Flux does not bind the tasks to specific cores. 
        # The tasks are launched, and they are free to run on any of the cores allocated to the job.
    ...
    cmd += [bash, report_and_run] + shlex.split(command)
    ...
    for line in raw_output.strip().split('\n'):
        ...
        parts.pop(0)
        print(" " + " ".join(parts))
        rank_id, node_name, cpuset, numa_domain = parts
        rank_id = int(rank_id)
        
        # Convert the cpuset (e.g., '0x00000001') to a physical core ID
        try:
            core_id_str = subprocess.run(["hwloc-calc", "--pulist", cpuset], check=True, capture_output=True, text=True).stdout.strip()
            print(f"hwloc-calc --pulist {cpuset}")
            actual_layout[rank_id] = cpuset + f" ({core_id_str})"
        except (subprocess.CalledProcessError, ValueError):
            print(f"Warning: Could not parse core ID for rank {rank_id} with cpuset {cpuset}")
...
def validate_results(predictions, actual_layout):
    ...
    for rank in all_ranks:
        ...
        if predicted_core == actual_core:
            print(f"🟢 Rank {rank} - Correctly bound to physical core {actual_core}")
        else:
            print(f"🔴 Rank {rank} - Predicted cpuset: {predicted_core}, Actual cpuset: {actual_core}")
            mismatches += 1
...
def get_parser():
    ...
    parser.add_argument(
        "--command",
        help="Command to run (or script)",
        default="kripke --procs 1,2,1",
    )
    parser.add_argument(
        "--match-policy",
        default="high",
    ...
    parser.add_argument("-c", "--cores-per-task", type=int, default=None, help="The number of CORES (not PUs) to bind per task.")
    return parser
...
def main():    
    ...
    predictions = predict_layout(topology, args.tasks, args.cores_per_task, args.match_policy)
    actual_layout = run_and_capture(args.command, args.nodes, args.tasks, exclusive=args.exclusive, cores_per_task=args.cores_per_task)
